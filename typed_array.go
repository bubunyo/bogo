...
func encodeTypedArray(arr any) ([]byte, error) {
	v := reflect.ValueOf(arr)

	if v.Kind() != reflect.Slice && v.Kind() != reflect.Array {
		return nil, fmt.Errorf("typed array encoder: type is not array/slice")
	}

	if v.Len() == 0 {
		// Empty array - encode as regular array
	...
	for i := 1; i < v.Len(); i++ {
		...
		if reflect.TypeOf(elem) != reflect.TypeOf(firstElem) {
			// Mixed types - fall back to regular array
			return encodeArray(arr)
		}
	}
	...
	default:
		// Unsupported type for optimization - fall back to regular array
		return encodeArray(arr)
	}
	...
	// Build final typed array: TypeTypedArray + LenSize + DataSize + ElementType + Count + Elements
	buf.WriteByte(TypeTypedArray)
...
func decodeTypedArray(data []byte) (any, error) {
	if len(data) < 2 {
		return nil, fmt.Errorf("typed array decode error: insufficient data for size")
	}

	// Read the total data size
	sizeLen := int(data[0])
	if len(data) < sizeLen+1 {
		return nil, fmt.Errorf("typed array decode error: insufficient data for size info")
	}

	totalSize, err := decodeUint(data[1 : 1+sizeLen])
	if err != nil {
		return nil, fmt.Errorf("typed array decode error: %w", err)
	}

	dataStart := 1 + sizeLen
	dataEnd := dataStart + int(totalSize)
	if len(data) < dataEnd {
		return nil, fmt.Errorf("typed array decode error: insufficient data for content")
	}

	arrayData := data[dataStart:dataEnd]

	if len(arrayData) < 1 {
		return nil, fmt.Errorf("typed array decode error: no element type")
	}

	// Read element type
	elementType := Type(arrayData[0])
	arrayData = arrayData[1:]

	// Read count
	if len(arrayData) < 1 {
		return nil, fmt.Errorf("typed array decode error: no count info")
	}

	countLen := int(arrayData[0])
	if len(arrayData) < countLen+1 {
		return nil, fmt.Errorf("typed array decode error: insufficient count data")
	}

	count, err := decodeUint(arrayData[1 : 1+countLen])
	if err != nil {
		return nil, fmt.Errorf("typed array decode error: failed to decode count: %w", err)
	}

	elementsData := arrayData[1+countLen:]
	...
	case TypeString:
		...
		for i := uint64(0); i < count; i++ {
			if pos >= len(elementsData) {
				return nil, fmt.Errorf("typed array decode error: insufficient string data at index %d", i)
			}

			strLenSize := int(elementsData[pos])
			pos++
			if pos+strLenSize > len(elementsData) {
				return nil, fmt.Errorf("typed array decode error: insufficient string length data")
			}

			strLen, err := decodeUint(elementsData[pos : pos+strLenSize])
			if err != nil {
				return nil, fmt.Errorf("typed array decode error: failed to decode string length: %w", err)
			}
			pos += strLenSize

			if pos+int(strLen) > len(elementsData) {
				return nil, fmt.Errorf("typed array decode error: insufficient string content data")
			}
	...
	case TypeByte:
		if len(elementsData) != int(count) {
			return nil, fmt.Errorf("typed array decode error: byte array size mismatch")
		}
	...
	case TypeInt:
		...
		for i := uint64(0); i < count; i++ {
			if pos >= len(elementsData) {
				return nil, fmt.Errorf("typed array decode error: insufficient int data")
			}

			intLenSize := int(elementsData[pos])
			pos++
			if pos+intLenSize > len(elementsData) {
				return nil, fmt.Errorf("typed array decode error: insufficient int length data")
			}

			val, err := decodeInt(elementsData[pos : pos+intLenSize])
			if err != nil {
				return nil, fmt.Errorf("typed array decode error: failed to decode int: %w", err)
			}
			result[i] = val
			pos += intLenSize
		}
	...
	case TypeUint:
		...
		for i := uint64(0); i < count; i++ {
			if pos >= len(elementsData) {
				return nil, fmt.Errorf("typed array decode error: insufficient uint data")
			}

			uintLenSize := int(elementsData[pos])
			pos++
			if pos+uintLenSize > len(elementsData) {
				return nil, fmt.Errorf("typed array decode error: insufficient uint length data")
			}

			val, err := decodeUint(elementsData[pos : pos+uintLenSize])
			if err != nil {
				return nil, fmt.Errorf("typed array decode error: failed to decode uint: %w", err)
			}
			result[i] = val
			pos += uintLenSize
		}
	...
	case TypeFloat:
		...
		for i := uint64(0); i < count; i++ {
			if pos >= len(elementsData) {
				return nil, fmt.Errorf("typed array decode error: insufficient float data")
			}

			floatLenSize := int(elementsData[pos])
			pos++
			if pos+floatLenSize > len(elementsData) {
				return nil, fmt.Errorf("typed array decode error: insufficient float length data")
			}

			val, err := decodeFloat(elementsData[pos : pos+floatLenSize])
			if err != nil {
				return nil, fmt.Errorf("typed array decode error: failed to decode float: %w", err)
			}
			result[i] = val
			pos += floatLenSize
		}
	...
	case TypeBoolTrue:
		...
		if len(elementsData) != int(count) {
			return nil, fmt.Errorf("typed array decode error: bool array size mismatch")
		}
	...
	default:
		return nil, fmt.Errorf("typed array decode error: unsupported element type: %d", elementType)
	}
}
